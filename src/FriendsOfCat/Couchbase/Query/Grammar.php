...
{
    public const INDEX_TYPE_VIEW = 'VIEW';
    public const INDEX_TYPE_GSI = 'GSI';
    public const IDENTIFIER_ENCLOSURE_CHAR = '`';
    public const VIRTUAL_META_ID_COLUMN = '_id';
    ...
    public function wrapValue($value)
    {
        ...
        if (self::IDENTIFIER_ENCLOSURE_CHAR === mb_substr(
            $value,
            0,
            1
        ) && self::IDENTIFIER_ENCLOSURE_CHAR === mb_substr($value, -1)) {
        ...
        return self::IDENTIFIER_ENCLOSURE_CHAR . str_replace(
            self::IDENTIFIER_ENCLOSURE_CHAR,
    ...
    public function applyBindings(string $sql, array $bindings)
    {
        ...
        while (! empty($sql)) {
            ...
            $firstPos = $literalSinglequotePos !== false ? ($firstPos === null ? $literalSinglequotePos : min(
                $literalSinglequotePos,
                $firstPos
            )) : $firstPos;
            $firstPos = $literalDoublequotePos !== false ? ($firstPos === null ? $literalDoublequotePos : min(
                $literalDoublequotePos,
                $firstPos
            )) : $firstPos;
            ...
            if ($identifierPos === $firstPos) {
                [$identifier, $sql] = $shiftIdentifier($sql);
                $tokens[] = [
                    'context' => 'identifier',
                    'sql' => $identifier,
                ];
            } elseif ($literalSinglequotePos === $firstPos) {
                [$literal, $sql] = $shiftLiteralSinglequote($sql);
                $tokens[] = [
                    'context' => 'literal',
                    'sql' => $literal,
                ];
            } elseif ($literalDoublequotePos === $firstPos) {
                [$literal, $sql] = $shiftLiteralDoublequote($sql);
                $tokens[] = [
                    'context' => 'literal',
                    'sql' => $literal,
                ];
